...
- **Real, selectable text** — glyph outlines are re-emitted as embeddable fonts
  ...
  (The OCD-EPUB container stores every font once in `pages/fonts.svg` — outlines,
...
- **Three web apps** (zero-dependency, JDK HTTP server, no JavaFX) sharing one
  ...
  - `Prism` — the jexter document workbench: open a PDF (the engine lifts it) or an
    ...
    a **Pages** tab — organise the document on a light table (move, insert a blank,
    duplicate, delete; the engine remaps the outline, the links and the structure),
    crop a page with handles or by the numbers, turn it —
...
- **Redaction, proven** (`sugarcube.jexter.redact`) — two verbs on one model.
  **Audit** — *is this PDF actually redacted?* A geometric query, no rendering: an
  opaque box painted **over** text or an image (paint order is the test) is
  reported with the hidden text itself as the proof; `/Redact` annotations never
  applied are reported too. **Redact** — nothing is covered, what is under a zone
  is *removed*: glyphs, vector fills, image **pixels**, annotations, links,
  ...
  proves the zones clean and returns just the changed members — those pages,
  their images, `structures.json` — which are put into the open container and
  reloaded. Export applies whatever is still pending, in one pass. `--redactMeta`
  also clears title, subject, authors, keywords and custom metadata.
...
- No jar for JPEG 2000: PDFBox needs the JAI tools for `JPXDecode` and they are not
  shipped, so the JJ2000 codec is vendored as source in `src/jj2000/` and compiles
  with the tree. Nothing to install.
- BouncyCastle is only needed at runtime to open encrypted PDFs (optional).

## Build

`nimbus-jexter.cjs` is the build — zero npm dependencies, straight `javac`/`jar`/`jpackage`,
...
java -cp "$CP" sugarcube.jexter.pdfimport.PdfImporter in.pdf 0 out.png 144
...
# Page set — the pages the output holds, stated as the RESULT (1-based): omission deletes, order
# reorders, repetition duplicates, "+" inserts a blank. Composes with every target.
java -cp "$CP" sugarcube.jexter.write.Conversion in.pdf pages1-3.pdf --pages=1-3      # extract
java -cp "$CP" sugarcube.jexter.write.Conversion book.ocd.epub out.ocd.epub --pages=6,1,+,3-4
...
# Redaction — audit (exit 1 = recoverable) and apply (written only when the result audits clean)
java -cp "$CP" sugarcube.jexter.tool.Redact audit in.pdf [--json]
java -cp "$CP" sugarcube.jexter.tool.Redact apply in.pdf out.pdf                 # --redact=audit: repair the boxes
java -cp "$CP" sugarcube.jexter.tool.Redact apply in.pdf out.pdf --redact="1:160,696,66,14;3:72,700,200,12"
# the same through the one entry point (and /api/convert): --to=audit, --redact=… on any target
java -cp "$CP" sugarcube.jexter.write.Conversion in.pdf audit.json --to=audit
java -cp "$CP" sugarcube.jexter.write.Conversion in.pdf out.md --redact=audit
java -cp "$CP" sugarcube.jexter.write.Conversion in.pdf zones.json --to=zones --match='\d{4}-\d{4}'   # every match, as zones
...
`nimbus-jexter.cjs` at the root is the build orchestrator — zero npm dependencies, run
it with `node nimbus-jexter.cjs`. Everything else is the engine, all under
`sugarcube.jexter`:
...
| `pdfimport` | `PdfImporter` + `PdfStreamEngine` (paths, glyph runs, MCID tracking), `FontExtractor`, `TaggedStructureBuilder` (PDF/UA), `PdfRenderer`, `ConvertOptions`. The only PDFBox-coupled **import** layer (`write.PdfWriter` uses PDFBox for PDF export). |
...
logical `OCDStructNode` tree are separate (the latter references content and never
alters pixels).
...
- **[doc/FORMAT.md](doc/FORMAT.md)** — the **OCD-EPUB** working format, normative.
  *Part A* is the container (OCF layout, manifest, the `ocd/` JSON members field
  by field); *Part B* is the page & font grammar, the strongly-typed SVG dialect
  the pages are written in (`data-version="2"`).
- **[doc/ANALYSIS.md](doc/ANALYSIS.md)** — the geometry-first structure recovery
  ...
  recovery, and what is deliberately deferred. `ocd/analysis/Analysis.java` remains the
...
- **JJ2000** — the JPEG 2000 codec, vendored as source under `src/jj2000/`
  (159 files, the decode path only — no encoder, no image writers). Copyright © 1999/2000 JJ2000 Partners — see
...
2.0) and vendors the JJ2000 JPEG 2000 codec as source; their notices must be
preserved on redistribution, under either license above — see [NOTICE](NOTICE).
...
Typical cases: PDF/UA remediation services and redaction / compliance workflows
built on the engine, embedded document pipelines, hosted converters.
