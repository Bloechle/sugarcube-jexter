...
/**
 * This class resamples the components of an image so that all have the same number of samples. The current implementation only handles the case of 2:1
 * upsampling.
 *
...
public class Resampler extends ColorSpaceMapper
{
   ...
   *
   * Note the present implementation does only two to one respampling in either direction (row, column).
   *
   * @param src -- Source of image data
  ...
  /**
   * Return a DataBlk containing the requested component upsampled by the scale factor applied to the particular scaling direction
   *
