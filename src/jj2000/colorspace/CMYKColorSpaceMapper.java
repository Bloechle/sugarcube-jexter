 ...
 * @author	Bruce A. Kern
 */
public class CMYKColorSpaceMapper extends ColorSpaceMapper
{
  /* sYCC colorspace matrix */
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix00 = 1;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix01 = 0;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix02 = (float) 1.402;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix10 = 1;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix11 = (float) -0.34413;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix12 = (float) -0.71414;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix20 = 1;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix21 = (float) 1.772;
  /**
   * Matrix component for ycc transform.
   */
  static protected float Matrix22 = 0;
  ...
  {
    return new CMYKColorSpaceMapper(src, csMap);
  }
   ...
   */
  protected CMYKColorSpaceMapper(BlkImgDataSrc src, ColorSpace csMap) throws ColorSpaceException
  {
    super(src, csMap);
    initialize();
  ...
  private void initialize() throws ColorSpaceException
  {
    ...
    System.out.println("CMYKColorSpaceMapper.initialize - ncomps=" + ncomps);
    if (ncomps != 4)
    {
      String msg = "SYccColorSpaceMapper: ycc transformation _not_ applied to " + ncomps + " component image";
  ...
  /**
   * Output a DataBlkFloat array where each sugarcube.app.sample in each component is the product of the YCC matrix * the vector of samples across the input components.
   *
  ...
  /**
   * Output a DataBlkInt array where each sugarcube.app.sample in each component is the product of the YCC matrix * the vector of samples across the input components.
   *
  ...
  private static DataBlkInt[] mult(DataBlkInt[] inblk)
  {
    ...
    for (j = 0; j < length; ++j)
    {
      out[0][j] = (int) (Matrix00 * in[0][inblk[0].offset + j]
        + Matrix01 * in[1][inblk[1].offset + j]
        + Matrix02 * in[2][inblk[2].offset + j]);

      out[1][j] = (int) (Matrix10 * in[0][inblk[0].offset + j]
        + Matrix11 * in[1][inblk[1].offset + j]
        + Matrix12 * in[2][inblk[2].offset + j]);

      out[2][j] = (int) (Matrix20 * in[0][inblk[0].offset + j]
        + Matrix21 * in[1][inblk[1].offset + j]
        + Matrix22 * in[2][inblk[2].offset + j]);
    }
