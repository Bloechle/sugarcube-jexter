 ...
 * image. The default implementation is just to return the value of the source at the current image resolution level, which is set by the 'setImgResLevel()'
 * method.
 *
 * <p>This abstract class can be used to facilitate the development of other classes that implement the 'InvWT' interface, because most of the trivial methods
 * are already implemented.</p>
 *
 * <p>If the default implementation of a method provided in this class does not suit a particular implementation of the 'InvWT' interface, the method can be
 * overriden to implement the proper behaviour.</p>
 *
 * <p>If the 'setImgResLevel()' method is overriden then it is very important that the one of this class is called from the overriding method, so that the other
 * methods in this class return the correct values.</p>
 *
 * @see InvWT
 *
 */
public abstract class InvWTAdapter implements InvWT
{
  ...
  /**
   * Instantiates the 'InvWTAdapter' object using the specified 'MultiResImgData' source. The reconstruction resolution level is set to full resolution (i.e.
   ...
   *
   * <p>This default implementation returns the value of the source at the current reconstruction resolution level.</p>
   *
