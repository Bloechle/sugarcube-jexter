...
/**
 * This class implements a wrapper to turn an InputStream into a RandomAccessIO. To provide random access, the input data from the InputStream is cached in an
 * in-memory buffer. The in-memory buffer size can be limited to a specified size. The data is read into the cache on a as needed basis, blocking only when
 * necessary.
 *
 * <p>The cache grows automatically as necessary. However, if the data length is known prior to the creation of a ISRandomAccessIO object, it is best to specify
 * that as the initial in-memory buffer size. That will minimize data copying and multiple allocation.<p>
 *
 * <p>Multi-byte data is read in big-endian order. The in-memory buffer storage is released when 'close()' is called. This class can only be used for data
 * input, not output. The wrapped InputStream is closed when all the input data is cached or when 'close()' is called.</p>
 *
 * <p>If an out of memory condition is encountered when growing the in-memory buffer an IOException is thrown instead of an OutOfMemoryError. The exception
 * message is "Out of memory to cache input data".</p>
 *
 * <p>This class is intended for use as a "quick and dirty" way to give network connectivity to RandomAccessIO based classes. It is not intended as an efficient
 * means of implementing network connectivity. Doing such requires reimplementing the RandomAccessIO based classes to directly use network connections.</p>
 *
 * <p>This class does not use temporary files as buffers, because that would preclude the use in unsigned applets.</p>
 *
 */
public class ISRandomAccessIO implements RandomAccessIO
{
  ...
  /**
   * Creates a new RandomAccessIO wrapper for the given InputStream 'is'. The internal cache buffer will have size 'size' and will increment by 'inc' each time
   * it is needed. The maximum buffer size is limited to 'maxsize'.
   *
  ...
  /**
   * Creates a new RandomAccessIO wrapper for the given InputStream 'is'. The internal cache buffer size and increment is to to 256 kB. The maximum buffer size
   * is set to Integer.MAX_VALUE (2 GB).
   *
   * @param is The input from where to get the data.
     *
   */
   ...
   *
   * @exception IOException If the maximum cache size is reached or if not enough memory is available to grow the buffer.
     *
   */
  ...
  /**
   * Reads data from the wrapped InputStream and places it in the cache buffer. Reads all input data that will not cause it to block, but at least on byte is
   * read (even if it blocks), unless EOF is reached. This method can not be called if EOF has been already reached (i.e. 'complete' is true). The wrapped
   * InputStream is closed if the EOF is reached.
   *
   * @exception IOException An I/O error occurred, out of meory to grow cache or maximum cache size reached.
     *
   */
   ...
   *
   * @exception IOException If an I/O error occurs while closing the underlying InputStream.  
     *
   */
  ...
  /**
   * Moves the current position for the next read operation to offset. The offset is measured from the beginning of the stream. If the offset is set beyond the
   * currently cached data, the missing data will be read only when a read operation is performed. Setting the offset beyond the end of the data will cause an
   * EOFException only if the data length is currently known, otherwise an IOException will occur when a read operation is attempted at that position.
   *
   * @param off The offset where to move to.
   *
   * @exception EOFException If seeking beyond EOF and the data length is known.
   *
  ...
  /**
   * Returns the length of the stream. This will cause all the data to be read. This method will block until all the data is read, which can be lengthy across
   * the network.
   *
  ...
  /**
   * Reads 'len' bytes of data from this file into an array of bytes. This method reads repeatedly from the stream until all the bytes are read. This method
