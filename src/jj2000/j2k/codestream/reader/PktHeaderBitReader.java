...
class PktHeaderBitReader
{
  ...
  /**
   * Reads a specified number of bits and returns them in a single integer. The bits are returned in the 'n' least significant bits of the returned integer. The
   * maximum number of bits that can be read is 31.
   *
  ...
  /**
   * Synchronizes this object with the underlying byte based input. It discards and buffered bits and gets ready to read bits from the current position in the
   * underlying byte based input.
   *
   * <p>This method should always be called when some data has been read directly from the underlying byte based input since the last call to 'readBits()' or
   * 'readBit()' before a new call to any of those methods.</p>
     *
   */
  void sync()
  {
   ...
   *
   * <p>This method is equivalent to creating a new 'PktHeaderBitReader' object.</p>
   *
   * @param in The source of byte data
     *
   */
