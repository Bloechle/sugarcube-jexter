...
/**
 * This class implements the tag tree decoder. A tag tree codes a 2D matrix of integer elements in an efficient way. The decoding procedure 'update()' updates a
 * value of the matrix from a stream of coded data, given a threshold. This procedure decodes enough information to identify whether or not the value is greater
 * than or equal to the threshold, and updates the value accordingly.
 *
 * <p>In general the decoding procedure must follow the same sequence of elements and thresholds as the encoding one. The encoder is implemented by the
 * TagTreeEncoder class.</p>
 *
...
public class TagTreeDecoder
{
  ...
  /**
   * Creates a tag tree decoder with 'w' elements along the horizontal dimension and 'h' elements along the vertical direction. The total number of elements is
   * thus 'vdim' x 'hdim'.
   *
   * <p>The values of all elements are initialized to Integer.MAX_VALUE (i.e. no information decoded so far). The states are initialized all to 0.</p>
   *
  ...
  /**
   * Decodes information for the specified element of the tree, given the threshold, and updates its value. The information that can be decoded is whether or
   * not the value of the element is greater than, or equal to, the value of the threshold.
   *
   ...
   *
   * @exception EOFException If the ned of the 'in' stream is reached before getting all the necessary data.
     *
   */
      ...
      while (t > ts)
        if (tv >= ts) // We are not done yet
          if (in.readBit() == 0) // '0' bit
            ...
            // We know that 'value' = treeS[k][idx]
            tv = ts++; // Increment of treeS[k][idx] done above
        else
