...
/**
 * This class apply inverse component transformations to the tiles depending on specification read from the codestream header. These transformations can be used
 * to improve compression efficiency but are not related to colour transforms used to map colour values for display purposes. JPEG 2000 part I defines 2
 * component transformations: RCT (Reversible Component Transformation) and ICT (Irreversible Component Transformation).
 *
 * @see ModuleSpec
 *
 */
public class InvCompTransf extends ImgDataAdapter implements BlkImgDataSrc
{
  ...
  /**
   * The list of parameters that is accepted by the inverse component transformation module. They start with 'M'.
   */
    ...
    int[] utdepth, ParameterList pl)
  {
    ...
    noCompTransf = !(pl.bool("comp_transf"));
  }
  ...
  /**
   * Returns a string with a descriptive text of which inverse component transformation is used. This can be either "Inverse RCT" or "Inverse ICT" or "No
   ...
   *
   * <P>This default implementation assumes that the number of fractional bits is not modified by the component mixer.
   *
   ...
   * <i>b</b> then for unsigned data the nominal range is between 0 and 2^b-1, and for signed data it is between -2^(b-1) and 2^(b-1)-1.
   *
   * @param c The index of the component.
   *
  ...
  /**
   * Apply inverse component transformation associated with the current tile. If no component transformation has been requested by the user, data are not
   * modified.
   *
   * <P>This method calls the getInternCompData() method, but respects the definitions of the getCompData() method defined in the BlkImgDataSrc interface.
   *
  ...
  /**
   * Apply the inverse component transformation associated with the current tile. If no component transformation has been requested by the user, data are not
   * modified. Else, appropriate method is called (invRCT or invICT).
   *
   * @see #invRCT
   *
   * @see #invICT
  ...
  /**
   * Apply inverse component transformation to obtain requested component from specified block of data. Whatever the type of requested DataBlk, it always
  ...
  /**
   * Apply inverse irreversible component transformation to obtain requested component from specified block of data. Whatever the type of requested DataBlk, it
   ...
   *
   * <P>This default implementation changes the tile in the source and re-initializes properly component transformation variables..
   *
   ...
   *
   * <P>This default implementation just advances to the next tile in the source and re-initializes properly component transformation variables.
   *
   *
   */
