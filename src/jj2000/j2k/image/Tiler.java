...
/**
 * This class places an image in the canvas coordinate system, tiles it, if so specified, and performs the coordinate conversions transparently. The source must
 * be a 'BlkImgDataSrc' which is not tiled and has a the image origin at the canvas origin (i.e. it is not "canvased"), or an exception is thrown by the
 * constructor. A tiled and "canvased" output is given through the 'BlkImgDataSrc' interface. See the 'ImgData' interface for a description of the canvas and
 * tiling.
 *
 * <p>All tiles produced are rectangular, non-overlapping and their union covers all the image. However, the tiling may not be uniform, depending on the nominal
 * tile size, tiling origin, component subsampling and other factors. Therefore it might not be assumed that all tiles are of the same width and height.</p>
 *
 * <p>The nominal dimension of the tiles is the maximal one, in the reference grid. All the components of the image have the same number of tiles.</p>
 *
 * @see ImgData
 * @see BlkImgDataSrc
 *
...
public class Tiler extends ImgDataAdapter implements BlkImgDataSrc
{
  ...
  private int yt0siz;
  /**
   * The nominal width of the tiles, on the reference grid. If 0 then there is no tiling in that direction.
   */
  private int xtsiz;
  /**
   * The nominal height of the tiles, on the reference grid. If 0 then there is no tiling in that direction.
   */
  private int ytsiz;
  /**
   ...
   *
   * @param src The 'BlkImgDataSrc' source from where to get the image data. It must not be tiled and the image origin must be at '(0,0)' on its canvas.
   *
   * @param ax The horizontal coordinate of the image origin in the canvas system, on the reference grid (i.e. the image's top-left corner in the reference
   * grid).
   *
   * @param ay The vertical coordinate of the image origin in the canvas system, on the reference grid (i.e. the image's top-left corner in the reference grid).
   *
   * @param px The horizontal tiling origin, in the canvas system, on the reference grid. It must satisfy 'px<=ax'.
   *

   * @param py The vertical tiling origin, in the canvas system, on the reference grid. It must satisfy 'py<=ay'.
   *

   * @param nw The nominal tile width, on the reference grid. If 0 then there is no tiling in that direction.
   *
   * @param nh The nominal tile height, on the reference grid. If 0 then there is no tiling in that direction.
   *
   * @exception IllegalArgumentException If src is tiled or "canvased", or if the arguments do not satisfy the specified constraints.
     *
   */
  public Tiler(BlkImgDataSrc src, int ax, int ay, int px, int py, int nw, int nh)
  {
    ...
    // dimension.
    if (xtsiz == 0)
      xtsiz = x0siz + src.getImgWidth() - xt0siz;
    if (ytsiz == 0)
      ytsiz = y0siz + src.getImgHeight() - yt0siz;
   ...
   * be modified by the caller then the other method is probably preferable.</p>
   *
   ...
   *
   * <p>The returned data may have its 'progressive' attribute set. In this case the returned data is only an approximation of the "final" data.</p>
   *
   ...
   * data is to be modified by the caller then this method is preferable.</p>
   *
