...
/**
 * This is the abstract class from which all dequantizers must inherit. This class has the concept of a current tile and all operations are performed on the
 * current tile.
 *
 ...
 * dimensions, and the tiles, are not modifed by the dequantizer. If that is not the case for a particular implementation then the methods should be
 * overriden.</p>
 *
 ...
 *
 * <p>The output data is either in floating-point, or in fixed-point two's complement. In case of floating-point data the the value returned by getFixedPoint()
 * must be 0. If the case of fixed-point data the number of fractional bits must be defined at the constructor of the implementing class and all operations must
 * be performed accordingly. Each component may have a different number of fractional bits.</p>
 *
 */
  ...
  implements CBlkWTDataSrcDec
{
   ...
   *
   * @param rb The number of "range bits" for each component (must be the "range bits" of the un-transformed components. For a definition of "range bits" see
   * the getNomRangeBits() method.
   *
   * @see #getNomRangeBits
     *
   */
   ...
   *
   * <P>The number of magnitude bits ('magBits' member variable) for each subband may have not been not initialized (it depends on the actual dequantizer and
   * its implementation). However, they are not necessary for the subsequent steps in the decoder chain.
   *
