...
/**
 * This class defines a Buffered Random Access File. It implements the <tt>BinaryDataInput</tt> and <tt>BinaryDataOutput</tt> interfaces so that binary data
 * input/output can be performed. This class is abstract since no assumption is done about the byte ordering type (little Endian, big Endian). So subclasses
 * will have to implement methods like <tt>readShort()</tt>, <tt>writeShort()</tt>, <tt>readFloat()</tt>, ...
 *
 * <P><tt>BufferedRandomAccessFile</tt> (BRAF for short) is a <tt>RandomAccessFile</tt> containing an extra buffer. When the BRAF is accessed, it checks if the
 * requested part of the file is in the buffer or not. If that is the case, the read/write is done on the buffer. If not, the file is uppdated to reflect the
 * current status of the buffer and the file is then accessed for a new buffer containing the requested byte/bit.
 *
 * @see RandomAccessIO
  ...
  implements RandomAccessIO, EndianType
{
  ...
  /**
   * Writes the byte value of <tt>v</tt> (i.e., 8 least significant bits) to the output. Prior to writing, the output should be realigned at the byte level.
   *
  ...
  /**
   * Reads an unsigned byte (i.e., 8 bit) from the input. It is returned as an <tt>int</tt> since Java does not have an unsigned byte type. Prior to reading,
   ...
   * which would be decided at creation time.
   *
   ...
   *
   * @exception java.io.EOFException If the end-of file was reached before all the bytes could be skipped.
   *
   * @exception java.io.IOException If an I/O error ocurred.
     *
   */
