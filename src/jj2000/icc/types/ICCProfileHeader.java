...
/**
 * An ICC profile contains a 128-byte header followed by a variable number of tags contained in a tag table. This class models the header portion of the
 * profile. Most fields in the header are ints. Some, such as data and version are aggregations of ints. This class provides an api to those fields as well as
 * the definition of standard constants which are used in the header.
 *
...
public class ICCProfileHeader
{
  ...
  static
  {
    kdwProfileSignature = ICCProfile.getInt(new String("acsp").getBytes(), 0);
  }
  /**
   * Profile header signature
   */
  ...
  static
  {
    kdwProfileSigReverse = ICCProfile.getInt(new String("psca").getBytes(), 0);
  }
   ...
   * Size of the header
   */
  public final static int size = offReserved + 44 * ICCProfile.byte_size;
   ...
   * Header field
   */
  public int dwPlatformSignature;	// Primary platform for which this profile was created
  /**
   * Header field
   */
   ...
   * Header field
   */
  public ICCProfileVersion profileVersion; // Version of the profile format on which
  /**
   * Header field
   */
  public ICCDateTime dateTime;		 // Date and time of profile creation// this profile is based
  /**
   * Header field
   */
  ...
  public void write(RandomAccessFile raf) throws IOException
  {
    ...
    raf.seek(offProfileSize);
    raf.write(dwProfileSize);
    raf.seek(offCMMTypeSignature);
    raf.write(dwCMMTypeSignature);
    raf.seek(offProfileVersion);
    profileVersion.write(raf);
    raf.seek(offProfileClass);
    raf.write(dwProfileClass);
    raf.seek(offColorSpaceType);
    raf.write(dwColorSpaceType);
    raf.seek(offPCSType);
    raf.write(dwPCSType);
    raf.seek(offDateTime);
    dateTime.write(raf);
    raf.seek(offProfileSignature);
    raf.write(dwProfileSignature);
    raf.seek(offPlatformSignature);
    raf.write(dwPlatformSignature);
    raf.seek(offCMMFlags);
    raf.write(dwCMMFlags);
    raf.seek(offDeviceManufacturer);
    raf.write(dwDeviceManufacturer);
    raf.seek(offDeviceModel);
    raf.write(dwDeviceModel);
    raf.seek(offDeviceAttributes1);
    raf.write(dwDeviceAttributes1);
    raf.seek(offDeviceAttributesReserved);
    raf.write(dwDeviceAttributesReserved);
    raf.seek(offRenderingIntent);
    raf.write(dwRenderingIntent);
    raf.seek(offPCSIlluminant);
    PCSIlluminant.write(raf);
    raf.seek(offCreatorSig);
    raf.write(dwCreatorSig);
    raf.seek(offReserved);
    raf.write(reserved);
  }
