...
/**
 * The OCD-native analysis pipeline: it runs on <i>any</i> in-memory {@link OCDDocument} — whether it
 * came from a PDF or was read back from an OCD-EPUB.
 *
 * <p>It lived in {@code convert} until 2026-09-07, beside the PDF import it had been factored out of.
 * It imports nothing from there and nothing from PDFBox: what it orchestrates is the stages of THIS
 * package, so this is where it belongs and where a reader looks for it.
 ...
 * the PDF importer when importing a tagged document.
 *
...
public final class Analysis {
     ...
     *  the PDF importer, before the {@code pdf} structure is built); it drives the outline-priority gate. */
    public static void run(OCDDocument doc, ConvertOptions opts, boolean pdfTagged) {
        opts.at(sugarcube.jexter.core.JxProgress.Stage.ANALYSIS, 0, 0);   // one shot: it has no unit to count
