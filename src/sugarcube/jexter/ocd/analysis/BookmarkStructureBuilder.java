 ...
 * {@link OCDStructNode.Type#HEADING} at its tree depth, <b>anchored</b> to the page {@link Segmenter.Block} that
 ...
 * {@link OCDStructNode.Type#PARAGRAPH} under the heading that precedes them (the document root before the first
...
public final class BookmarkStructureBuilder {
    ...
    public static void build(OCDDocument doc) {
        ...
        Map<Block, OCDStructNode> anchored = new IdentityHashMap<>();                        // anchor block → its HEADING
    ...
    private static void walk(OCDOutline o, int level, OCDStructNode parent, Map<Integer, List<Block>> byPage,
                             int[] stats, Map<Block, OCDStructNode> anchored) {
        OCDStructNode h = new OCDStructNode(OCDStructNode.Type.HEADING).level(Math.min(MAX_LEVEL, level));
