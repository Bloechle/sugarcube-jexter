...
final class BlockSignals {
            ...
            if (n instanceof OCDText t) {
                ...
                if (run != null && !run.isEmpty()) {
                    ...
                    if (run.length() > s.domChars) {                       // longest run drives size/font/colour/angle
                        ...
                        // The run's angle is the direction of the image of the x-axis: (m00, m10),
                        // which JxTransform names (a, b) — NOT (a, c). Kept as two scalars because
                        // the signal is serialized, not the matrix.
                        if (tr != null) { s.domA = tr.a(); s.domC = tr.b(); }
                    }
    ...
    private static int[] gridBox(JxRect r, double pw, double ph) {
        return new int[] {
            clamp(r.minX() / pw), clamp((ph - r.maxY()) / ph),
