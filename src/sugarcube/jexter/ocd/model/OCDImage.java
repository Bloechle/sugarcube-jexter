...
public final class OCDImage extends OCDNode {
    ...
    /** A full copy. The pixels are a DOCUMENT resource addressed by {@link #resourceRef()}, so the two
     *  images share the bytes — which is what the resource table is for. */
    @Override public OCDImage copy() {
        OCDImage im = copyInto(new OCDImage(resourceRef));
        im.pixelWidth = pixelWidth; im.pixelHeight = pixelHeight;
        return im;
    }
