...
public final class OCDText extends OCDNode {
    ...
    /** A full copy — paint state and glyphs. {@link Glyph} is a record, so the list is copied and its
    ...
    /** {@code text().strip().length()} without building the string. The analysis weighs runs by how much
     *  ink they carry — {@code Segmenter.dominantSize} and {@code runStyle} ask once per run, and asked
     *  twice each while the string itself was thrown away: 2.5% of a conversion spent measuring a length. */
    public int strippedLength() {
        int total = 0, lead = 0, trail = 0;
        ...
        for (Glyph g : glyphs) {
            ...
            for (int i = 0; i < u.length(); i++) {
                total++;
                if (Character.isWhitespace(u.charAt(i))) { if (started) trail++; else lead++; }
                else { started = true; trail = 0; }
            }
        }
        return started ? total - lead - trail : 0;
    }
