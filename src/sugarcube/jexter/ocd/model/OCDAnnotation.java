...
public final class OCDAnnotation {
    ...
    /** The annotation kind — pure classification (no behaviour), like {@link OCDStructNode.Type}. */
    public enum Markup { HIGHLIGHT, UNDERLINE, STRIKEOUT, SQUIGGLY, NOTE, FREETEXT, STAMP, INK, LINE, SHAPE, REDACT, OTHER }
    ...
    /** A copy — the quad list is duplicated, the rects and the colour are values. */
    public OCDAnnotation copy() {
        OCDAnnotation a = new OCDAnnotation();
        a.type = type; a.rect = rect; a.contents = contents; a.author = author;
        a.modified = modified; a.color = color;
        a.quads.addAll(quads);
        return a;
    }
}
