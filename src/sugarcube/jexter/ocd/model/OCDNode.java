...
public sealed abstract class OCDNode permits OCDText, OCDPath, OCDImage, OCDGroup, OCDMedia, OCDBreak {
    ...
    // ── copy ───────────────────────────────────────────────────────────────────
    /**
     * An independent copy of this node — the whole subtree for a group. Declared here, and abstract,
     * so the compiler names every type that forgets it: a copy that silently drops a field is a
     * document that quietly differs from the one it was copied from.
     *
     * <p>Node ids are copied as they are. They are <b>page-local by contract</b>
     * ({@link IdStamper}), so a copy placed on another page keeps every structure reference valid;
     * a copy placed on the SAME page must be re-stamped by its caller.
     *
     * <p>The gate is byte equality of the projection: a page and its copy must serialize identically
     * ({@code SvgOcdWriter.render}). Anything that reaches the output and was not copied shows up
     * there immediately.
     */
    public abstract OCDNode copy();

    /** Copy the state every node shares, into a fresh node of the right type. */
    protected <T extends OCDNode> T copyInto(T n) {
        n.id = this.id;
        n.z = this.z;
        n.name = this.name;
        n.role = this.role;
        n.transform = this.transform;      // immutable
