...
abstract sealed class OtfBuilder permits CffOtf, GlyfOtf {
    ...
    /**
     * The PostScript name written into the compiled font — taken from the font's document-unique
     * ID, not its family name.
     *
     * <p>A PDF consumer identifies an embedded subset by its {@code /BaseFont}, and PDFBox builds
     * that from this name plus a six-letter tag it derives from the glyph mapping alone
     * ({@code TrueTypeEmbedder.getTag}: base-25 of {@code gidToCid.hashCode()}). Two subsets of the
     * same typeface with the same glyph count therefore get the SAME tag and, with a family name,
     * the same /BaseFont: a press page came out with 447 embedded subsets sharing 133 names — one
     * name covering 39 different font programs. The spec asks for a tag unique per subset; a reader
     * that caches by name has every reason to draw the wrong glyphs. The ID is unique by
     * construction, so the name is too.
     */
    final String psName() {
        String s = font.id() != null ? font.id()
                : font.name() != null ? font.name() : (font.family() != null ? font.family() : "Font");
