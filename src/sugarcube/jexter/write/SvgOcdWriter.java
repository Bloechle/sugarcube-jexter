 ...
 *   <li>{@code OCDText} run → {@code <g id data-ocd="run" data-font data-size data-text [data-chars] [data-blanks] class
 *       transform>}: the font size is FACTORED INTO the run matrix ({@code matrix(fs·…)}) so each
 *       painted glyph is one {@code <use href="#fN-gid" [x="em"]/>} (x omitted when 0); {@code data-text} is the unicode of
 *       the full stream (blanks included), {@code data-chars} the per-glyph char counts (only when
 *       non-uniform), {@code data-blanks} the never-painted sentinels as {@code at:xem} / {@code
 *       at:gid:xem} tokens;</li>
 *   <li>glyph outlines live ONCE per document in the shared sibling {@code pages/fonts.svg}
 *       (grouped per font: {@code <g id="fN" data-f>}); pages reference them via external
 *       {@code <use href="fonts.svg#fN-gid">} — the same pattern as the shared images;</li>
 ...
 *       file and carry {@code data-y}, the destination's y anchor);</li>
 ...
 * </ul>
 *
 * <p>Paint order = document order (z), as ever. Reading order is carried by {@code data-order}
...
public final class SvgOcdWriter {
    ...
    public static String render(OCDDocument doc, OCDPage page, Fonts glyphs) {
        ...
        double w = win.width(), h = win.height();
        ...
        box(sb, "data-trimbox",  page.trimBox());
        box(sb, "data-artbox",   page.artBox());
    ...
    /** Emits siblings in PAINT order (z), tagging {@code data-order} (content index) wherever the
    ...
    private static java.util.List<OCDNode> spliceGraphics(java.util.List<OCDNode> in) {
        ...
        for (OCDNode n : in) {
            // Un OCDGraphic neutre au rendu est DISSOUS, et ce n'est pas un oubli : ses enfants
            // n'occupent PAS une plage de z contiguë — d'autres contenus se peignent entre eux
            // (mesuré sur une une de presse : enfants z 287/225/255/316, voisins 131/196/283/221).
            // Les enfermer dans un <g> les forcerait dans un seul créneau de profondeur, et un
            // chemin passerait par-dessus le texte : vérifié au pixel dans un navigateur.
    ...
    private static void node(StringBuilder sb, OCDDocument doc, OCDPage page, OCDNode n, int order,
        ...
        switch (n) {
            ...
            case OCDParagraph para -> {
                sb.append("<g id=\"").append(para.id()).append("\" data-ocd=\"paragraph\"");
                o(sb, order);
                ...
                if (para.transform() != null && !para.transform().isIdentity())
                    sb.append(" data-matrix=\"").append(para.transform().toMatrix6()).append('"');
                state(sb, para);
                sb.append(">\n");
                ...
                for (OCDNode c : para.children()) {
                    ...
                    if (!open) { sb.append("<g data-ocd=\"line\">\n"); open = true; }
            ...
            case OCDMedia m  -> {
                ...
                sb.append(" data-kind=\"")
                  ...
                  .append("\" data-src=\"").append(esc(m.resourceRef())).append('"');
                ...
                if (m.transform() != null && !m.transform().isIdentity())
                    sb.append(" data-matrix=\"").append(m.transform().toMatrix6()).append('"');
            ...
            case OCDGroup g  -> {
                ...
                if (g.transform() != null && !g.transform().isIdentity())
                    sb.append(" data-matrix=\"").append(g.transform().toMatrix6()).append('"');
    ...
    private static String kind(OCDGroup g) {
        if (g instanceof OCDParagraph) return "paragraph";
        if (g instanceof OCDGraphic)   return "graphic";
    ...
    private static void text(StringBuilder sb, OCDDocument doc, OCDText t, OCDPage page, int order,
        ...
        sb.append("<g id=\"").append(t.id()).append("\" data-ocd=\"run\" data-font=\"").append(ff)
          .append("\" data-size=\"").append(f(fs)).append('"');
        o(sb, order);
        sb.append(" data-text=\"").append(sugarcube.jexter.core.JxText.attr(u)).append('"');
        if (!uniform) {
            sb.append(" data-chars=\"");
            boolean first = true;
            for (OCDText.Glyph g : t.glyphs()) {
                if (!first) sb.append(' ');
        ...
        for (OCDText.Glyph g : t.glyphs()) {
            ...
            if (!"0".equals(gx)) sb.append("\" x=\"").append(gx);
            sb.append("\"/>");
        }
        sb.append("</g>\n");
    }
    ...
    /** The shared font file ({@code pages/fonts.svg}) — since fonts.json died, this is THE
     *  single font representation: per font a {@code <g id="fN" data-font data-id …metrics…
     ...
     *  id="qN-gid" d data-advance [data-text] [data-glyphname]>} — inkless glyphs (spaces) keep an empty
    ...
    public static String glyphsSvg(OCDDocument doc, Fonts glyphs) {
        ...
        for (var fe : byName.entrySet()) {
            ...
            sb.append("<g id=\"").append(glyphs.alias.get(fe.getKey()))
              .append("\" data-font=\"").append(esc(fe.getKey()))
            ...
            sb.append(" data-ascent=\"").append(f(f.ascent())).append("\" data-descent=\"").append(f(f.descent()))
              .append("\" data-capheight=\"").append(f(f.capHeight())).append("\" data-xheight=\"").append(f(f.xHeight()))
              .append("\" data-space=\"").append(f(f.spaceWidth())).append('"');
            if (!f.cmap().isEmpty()) {
                sb.append(" data-cmap=\"");
                boolean first = true;
    ...
    /** The ONE glyph markup: {@code <path id="fN-gid" d data-advance [data-text] [data-glyphname]/>} —
     *  emitted identically into {@code pages/fonts.svg} (full library) and into each page's own
    ...
    private static void glyphPath(StringBuilder sb, String alias, OCDGlyph g) {
        sb.append("<path id=\"").append(alias).append('-').append(g.gid())
          ...
          .append("\" data-advance=\"").append(f(g.advance())).append('"');
        if (g.unicode() != null && !g.unicode().isEmpty())
            sb.append(" data-text=\"").append(sugarcube.jexter.core.JxText.attr(g.unicode())).append('"');
        if (g.name() != null && !g.name().isEmpty())
            sb.append(" data-glyphname=\"").append(sugarcube.jexter.core.JxText.attr(g.name())).append('"');
        sb.append("/>\n");
    }
    ...
    private static void o(StringBuilder sb, int order) {
        if (order >= 0) sb.append(" data-order=\"").append(order).append('"');
    }
