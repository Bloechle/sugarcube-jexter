 ...
 * {@link OCDStructNode} tree is walked in reading order:
...
public final class MarkdownWriter {
    ...
    private static final class Emit {
        ...
        void walk(OCDStructNode s) {
            if (s == null) return;
            switch (s.type()) {
                case DOCUMENT, SECTION, TOC -> { for (OCDStructNode c : s.children()) walk(c); }
                ...
                case FIGURE -> {
                    ...
                    for (OCDStructNode c : s.children()) walk(c);    // caption (if any) follows the placeholder
                }
                case TABLE -> { table(s); }
        ...
        void list(OCDStructNode s) {
            StringBuilder b = new StringBuilder();
            int n = 0;
            for (OCDStructNode c : s.children()) {
                if (c.type() == OCDStructNode.Type.ITEM) {
                    String t = stripMarker(text(c));
                    if (!t.isEmpty()) b.append(s.ordered() ? (++n + ". ") : "- ").append(t.replace("\n", " ")).append('\n');
                } else if (c.type() == OCDStructNode.Type.LIST) {     // nested list → indent
                    int m = 0;
                    for (OCDStructNode gc : c.children())
                        if (gc.type() == OCDStructNode.Type.ITEM) {
        ...
        void table(OCDStructNode s) {
            ...
            for (OCDStructNode c : s.children()) if (c.type() == OCDStructNode.Type.ROW) rows.add(c);
            if (rows.isEmpty()) return;
            int ncols = 0;
            List<List<String>> grid = new ArrayList<>();
            for (OCDStructNode r : rows) {
                List<String> row = new ArrayList<>();
                for (OCDStructNode c : r.children())
                    if (c.type() == OCDStructNode.Type.CELL) {
            ...
            sb.append(b).append('\n');
            for (OCDStructNode c : s.children())                 // table caption → italic line under the table
                if (c.type() == OCDStructNode.Type.CAPTION) { String t = text(c); if (!t.isEmpty()) block("*" + t + "*"); }
        }
        ...
        String text(OCDStructNode s) { return ix.text(s); }
    }
