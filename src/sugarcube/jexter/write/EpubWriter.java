...
public final class EpubWriter {
    ...
    /** The boolean overload, kept for callers outside this project: annotations only, defaults
     *  elsewhere — {@link ConvertOptions#RENDER_ANNOTATIONS} is what it sets. */
    public static void write(OCDDocument doc, OutputStream out, boolean annotations) throws IOException {
        write(doc, out, new ConvertOptions().set(ConvertOptions.RENDER_ANNOTATIONS, annotations));
    }

    public static void write(OCDDocument doc, OutputStream out, ConvertOptions opt) throws IOException {
        boolean annotations = Boolean.TRUE.equals(opt.get(ConvertOptions.RENDER_ANNOTATIONS));
        ...
        try (JxZip zip = new JxZip(out)) {
            ...
            EpubPackage.images(zip, doc);                                    // OEBPS/images/*
            ...
            nav(zip, doc, opt);                                  // OEBPS/nav.xhtml (logical: toc + page-list)
    ...
    private static void pagesXhtml(JxZip zip, OCDDocument doc, boolean annotations) throws IOException {
        ...
        for (OCDPage page : doc.pages()) {
            ...
            zip.deflated(OPF + "pages/" + OCDVocab.pageFile(i++) + ".xhtml", xhtml);
        }
    }

    // ── logical: EPUB3 navigation (toc + landmarks + page-list) ──────────────────

    private static void nav(JxZip zip, OCDDocument doc, ConvertOptions opt) throws IOException {
        OCDIndex ix = OCDIndex.of(doc);
        // navPerPage: every page in the nav, headings where they exist, "Page N" otherwise. A
        // projection choice, not a fact about the document — the model is never touched.
        String tocInner = Boolean.TRUE.equals(opt.get(ConvertOptions.NAV_PER_PAGE))
                ? EpubPackage.tocPerPage(doc, doc.structure(), ix,
                        p -> "pages/" + OCDVocab.pageFile(p + 1) + ".xhtml", p -> "Page " + (p + 1))
                : doc.structure() != null
