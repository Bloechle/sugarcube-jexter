...
public final class HtmlWriter {
    ...
    public static String body(OCDDocument doc, Assets assets) {
        ...
        var sb = new StringBuilder(16384);
        OCDStructNode root = doc.structure();
        if (root != null) for (OCDStructNode c : root.children()) render(c, doc, ix, sb, assets);
    ...
    public static List<Chapter> chapters(OCDDocument doc, Assets assets) {
        ...
        for (OCDStructNode c : root.children()) {
            if (c.type() == OCDStructNode.Type.HEADING && cur.length() > 0) {
                out.add(new Chapter("chapter-" + (out.size() + 1), curTitle != null ? curTitle : "Content", cur.toString()));
                cur = new StringBuilder();
                curTitle = null;
            }
            if (c.type() == OCDStructNode.Type.HEADING && curTitle == null) {
    ...
    private static void render(OCDStructNode s, OCDDocument doc, OCDIndex ix, StringBuilder sb, Assets assets) {
        switch (s.type()) {
            case HEADING -> {
                int lv = Math.min(6, Math.max(1, s.level() == 0 ? 1 : s.level()));
                line(sb, "h" + lv, content(s, ix));
                for (OCDStructNode c : s.children()) render(c, doc, ix, sb, assets);   // HTML5 outline: children are siblings
            }
            ...
            case CELL      -> wrap(s, doc, ix, sb, s.header() != OCDStructNode.HeaderKind.NONE ? "th" : "td", content(s, ix), assets);
    ...
    private static void wrap(OCDStructNode s, OCDDocument doc, OCDIndex ix,
        ...
        for (OCDStructNode c : s.children()) render(c, doc, ix, sb, assets);
        sb.append("</").append(tag).append(">\n");
    }

    private static void figure(OCDStructNode s, OCDDocument doc, OCDIndex ix, StringBuilder sb, Assets assets) {
        sb.append("<figure>\n");
        ...
        for (OCDStructNode c : s.children()) render(c, doc, ix, sb, assets);   // caption(s)
        sb.append("</figure>\n");
    }

    private static OCDMedia referencedMedia(OCDStructNode s, OCDIndex ix) {
    ...
    private static String content(OCDStructNode s, OCDIndex ix) {
        return ix.text(s);
    }
    ...
    private static String imageSrc(OCDStructNode s, OCDDocument doc, OCDIndex ix, Assets assets) {
        for (OCDStructNode.Ref r : s.refs()) {
            OCDNode n = ix.node(r.page(), r.nodeId());
    ...
    // Delegated, not re-implemented: JxText is THE XML-escaping rule for hand-built output, and it
    // does more than the three replacements this file used to carry — it also keeps XML-invalid
    // characters out, which a CID font with no usable ToUnicode produces by the handful (U+0000,
    // U+FFFF). Two escapers mean two behaviours the day one of them learns something.
    private static String text(String s) { return JxText.text(s); }
    private static String attr(String s) { return JxText.attr(s); }
