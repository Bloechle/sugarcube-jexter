...
/**
 * The {@code ocd/} JSON members of an OCD-EPUB — only what has NO home in the pages. Since
...
public final class OcdMembers {
    ...
    // ── annots member: ocd/annots.json (sparse — only pages that carry any) ──
    ...
    public static String annotsJson(OCDDocument doc) {
        ...
        for (OCDPage p : doc.pages()) {
            ...
            // Keyed by POSITION, like every other member and like the reader that parses these keys
            // back (OCDVocab.pageId ↔ pageIndex). It used to key by the page's own id, which agrees
            // only for as long as nothing moves a page: the ids are minted positionally at import and
            // at read, so after a reorder a page carries the id of where it USED to be, and its
            // annotations and form fields were written onto whichever page now holds that id.
            // Measured on the rich fixture reversed: two fields and two annotations came back on the
            // wrong page while the link on the same page was right. One authority for "which page".
            js.obj(OCDVocab.pageId(at));
            js.arr("annots");
    ...
    private static void struct(JxStringer js, OCDStructNode s) { struct(js, null, s); }

    private static void struct(JxStringer js, String key, OCDStructNode s) {
        ...
        if (s.header() != OCDStructNode.HeaderKind.NONE)  js.str("header", s.header().name().toLowerCase());
        ...
        if (!s.children().isEmpty()) {
            js.arr("children");
            for (OCDStructNode c : s.children()) struct(js, c);
            js.end();
        }
        js.end();
    }
