 ...
 *
 * <p>The source is the logical {@link OCDStructNode} tree
...
public final class DocTagsWriter {
    ...
    private static final class Emit {
        ...
        void walk(OCDStructNode s) {
            if (s == null) return;
            switch (s.type()) {
                case DOCUMENT, SECTION -> { for (OCDStructNode c : s.children()) walk(c); }  // containers
                case SPAN, OTHER -> {                           // inline under a text block is folded into its text(); at block position, emit it
                    if (hasBlockChild(s)) { for (OCDStructNode c : s.children()) walk(c); }
                    else block(s, "text");
                }
                case TOC      -> block(s, "document_index");
                case HEADING  -> { block(s, "section_header_level_" + clamp(s.level(), 1, 6));
                                   for (OCDStructNode c : s.children()) walk(c); }   // heading = label + sub-blocks
                ...
                case FIGURE   -> {                              // <picture> nests its caption (canonical)
                    ...
                    for (OCDStructNode c : s.children()) walk(c);
                    sb.append("</picture>\n");
                }
        ...
        void block(OCDStructNode s, String tag) {
            int page = pageOf(s);
        ...
        void table(OCDStructNode s) {
            int page = pageOf(s);
            JxRect box = subtreeBox(s, page);
            pageBreak(page);
            sb.append("<otsl>").append(loc(box, page)).append('\n');
            sb.append(otsl(s, page));
            for (OCDStructNode c : s.children())                 // caption encapsulated in the table block
                if (c.type() == OCDStructNode.Type.CAPTION) block(c, "caption");
            sb.append("</otsl>\n");
        }
        ...
        String otsl(OCDStructNode table, int page) {
            List<OCDStructNode> rows = new ArrayList<>();
            for (OCDStructNode c : table.children()) if (c.type() == OCDStructNode.Type.ROW) rows.add(c);
            int nrows = rows.size();
            if (nrows == 0) return "";
            int ncols = 0;
            for (OCDStructNode r : rows) { int w = 0; for (OCDStructNode c : r.children()) if (c.type() == OCDStructNode.Type.CELL) w += Math.max(1, c.colSpan()); ncols = Math.max(ncols, w); }
            if (ncols == 0) return "";
            ...
            for (int ri = 0; ri < nrows; ri++) {
                int ci = 0;
                for (OCDStructNode cell : rows.get(ri).children()) {
                    if (cell.type() != OCDStructNode.Type.CELL) continue;
                    ...
                    OCDStructNode.HeaderKind hk = cell.header();
                    String head = (hk == OCDStructNode.HeaderKind.COLUMN || hk == OCDStructNode.HeaderKind.BOTH) ? "ched"
                                : hk == OCDStructNode.HeaderKind.ROW ? "rhed" : "fcel";
        ...
        String text(OCDStructNode s, int page) { return ix.text(s, page); }

        /** Box of this node's own refs only (excludes child blocks) — for text/heading loc. */
        JxRect selfBox(OCDStructNode s, int page) {
            if (page < 0) return null;
            JxRect box = null;
            for (OCDStructNode.Ref r : s.refs()) {
        ...
        JxRect subtreeBox(OCDStructNode s, int page) {
            JxRect box = selfBox(s, page);
            for (OCDStructNode c : s.children()) {
                if (c.type() == OCDStructNode.Type.CAPTION) continue;
                JxRect cb = subtreeBox(c, page);
                if (cb != null) box = (box == null) ? cb : box.union(cb);
            }
            return box;
        }
        int pageOf(OCDStructNode s) {
            for (OCDStructNode.Ref r : s.refs()) return r.page();
            for (OCDStructNode c : s.children()) { int p = pageOf(c); if (p >= 0) return p; }
            return -1;
        }
    }
    ...
    private static boolean hasBlockChild(OCDStructNode s) {
        for (OCDStructNode c : s.children()) {
            OCDStructNode.Type t = c.type();
            if (t != OCDStructNode.Type.SPAN && t != OCDStructNode.Type.CODE && t != OCDStructNode.Type.OTHER) return true;
        }
        return false;
    }
