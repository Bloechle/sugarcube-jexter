...
- **Fidelity is a gate, not a goal.** The analysis layer is *additive*: it sets
  roles, reorders the content array into reading order, and builds an `OCDStructNode`
...
`node nimbus-jexter.cjs` opens a local build deck that does the same thing plus
