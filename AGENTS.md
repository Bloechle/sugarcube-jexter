...
description: >
  ...
  working on the OCD model, the PDF→OCD import (pdfimport), the analysis pass
...
```
PDF ─[pdfimport.PdfImporter]→ OCDDocument ─[analysis]→ structured OCDDocument
                            (the model)        │
...
- **`_` prefix = never committed.** `_prod/` holds the deliverables (`target/` is the intermediate scratch — the two are kept apart on purpose), `_private/` and `_ai-*` never leave the machine; every one of them is ignored, without exception. `nimbus-jexter.cjs` at the root is the build orchestrator — tracked, because it is the documented way to build the project. Everything else under `src/` is the engine.
...
- **Build** — `node nimbus-jexter.cjs` (deck) or `node nimbus-jexter.cjs --run=jar|build|context <abs path>`;
